...
**Events de log registrados no Kibana**
...
```bash
# Api
docker build -t template-dotnetcore-api . -f Richter.WhoAmIApi.API\Dockerfile
docker container run -e "ASPNETCORE_ENVIRONMENT=Development" -p 5001:5001 template-dotnetcore-api:latest

# Jobs
docker build -t template-dotnetcore-jobs . -f Richter.WhoAmIApi.Jobs\Dockerfile
